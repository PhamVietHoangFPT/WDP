...
export class TestResultShipmentRepository
    ...
    async findById(id: string): Promise<TestResultShipmentDocument | null> {
        return this.testResultShipmentModel.findOne({
            _id: id,
            deleted_at: null,
        }).exec()
    }
    ...
    async update(
        ...
        return this.testResultShipmentModel.findByIdAndUpdate(
            id,
            {
    ...
    async delete(id: string, userId: string): Promise<boolean> {
        const result = await this.testResultShipmentModel.findByIdAndUpdate(
            id,
