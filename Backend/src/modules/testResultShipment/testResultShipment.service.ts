...
export class TestResultShipmentService implements ITestResultShipmentService {
    ...
    async findAll(): Promise<TestResultShipmentResponseDto[]> {
        ...
        if (!shipments || shipments.length === 0) {
            throw new NotFoundException(
                'Không tìm thấy kiểu vận chuyển nào.',
            )
        }
    ...
    async update(
        ...
        if (!existingStatus) {
            throw new NotFoundException(
                `Không tìm thấy vận chuyển với ID ${id}.`,
            )
        }

        if (
            String(existingStatus.testResultDispatchedDate) === String(data.testResultDispatchedDate) &&
            String(existingStatus.testResultDeliveryDate) === String(data.testResultDeliveryDate)
        ) {
        ...
        if (!updatedShipment) {
            throw new NotFoundException(
                `Không thể cập nhật vận chuyển với ID ${id}.`,
            )
        }
