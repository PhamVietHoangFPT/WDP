...
export class RelationshipService implements IRelationshipService {
    ...
    private mapToResponseDto(relationship: Relationship): RelationshipResponseDto {
    ...
    async findRelationshipById(id: string): Promise<RelationshipResponseDto> {
        ...
        const existingRelationship = await this.relationshipRepository.findOneById(id)
    ...
    async createRelationship(userId: string, createRelationshipDto: CreateRelationshipDto): Promise<CreateRelationshipDto> {
        //this variable is used to check if the Relationship already exists
        const existingRelationship = await this.relationshipRepository.findOneByName(
    ...
    async updateRelationship(id: string, userId: string, updateRelationshipDto: UpdateRelationshipDto): Promise<any> {
        const existingRelationship = await this.findRelationshipById(id)
        if (
            existingRelationship.name === updateRelationshipDto.name &&
            existingRelationship.relationshipFee === updateRelationshipDto.relationshipFee &&
            existingRelationship.relationshipGap === updateRelationshipDto.relationshipGap &&
