...
export class ShipmentStatusService implements IShipmentStatusService {
    ...
    private mapToResponseDto(shipmentStatus: ShipmentStatus): ShipmentStatusResponseDto {
    ...
    async findShipmentStatusById(id: string): Promise<ShipmentStatusResponseDto> {
        ...
        const existingShipmentStatus = await this.shipmentStatusRepository.findOneById(id)
