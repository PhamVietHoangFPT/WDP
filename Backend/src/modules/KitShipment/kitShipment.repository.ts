...
export class KitShipmentRepository implements IKitShipmentRepository {
    ...
    async getCurrentStatusId(id: string): Promise<string | null> {
        const currentStatus = await this.kitShipmentModel.findOne({ _id: id })
            .select('currentStatus')
            .exec()
        // eslint-disable-next-line @typescript-eslint/no-base-to-string
        return currentStatus?.currentStatus ? currentStatus.currentStatus.toString() : null
    }

    async getCaseMemberId(id: string): Promise<string | null> {
        const caseMember = await this.kitShipmentModel.findOne({ _id: id })
            .select('caseMember')
            .exec()
        // eslint-disable-next-line @typescript-eslint/no-base-to-string
    ...
    async getSamplingKitInventoryId(id: string): Promise<string | null> {
        const samplingKitInventory = await this.kitShipmentModel.findOne({ _id: id })
            .select('samplingKitInventory')
            .exec()
        // eslint-disable-next-line @typescript-eslint/no-base-to-string
        return samplingKitInventory?.samplingKitInventory ? samplingKitInventory.samplingKitInventory.toString() : null
    }
    async getAddressId(id: string): Promise<string | null> {
        const address = await this.kitShipmentModel.findOne({ _id: id })
            .select('address')
            .exec()
        // eslint-disable-next-line @typescript-eslint/no-base-to-string
    ...
    async getDeliveryStaffId(id: string): Promise<string | null> {
        const deliveryStaff = await this.kitShipmentModel.findOne({ _id: id })
            .select('deliveryStaff')
            .exec()
        // eslint-disable-next-line @typescript-eslint/no-base-to-string
        return deliveryStaff?.deliveryStaff ? deliveryStaff.deliveryStaff.toString() : null
    }
    ...
    async findAll(): Promise<KitShipmentDocument[] | null> {
        return await this.kitShipmentModel
            ...
            .populate({
                path: 'samplingKitInventory', select: "_id ",
                populate: { path: 'facility', select: 'facilityName -_id' }
            })
            .populate({ path: 'address', select: 'fullAddress -_id' })
            .populate({ path: 'deliveryStaff', select: "-_id name email phoneNumber gender" })
            .exec()
    }
