...
import { KitShipmentStatus, KitShipmentStatusDocument } from './schemas/kitShipmentStatus.schema'
...
@Injectable()
export class KitShipmentStatusRepository implements IKitShipmentStatusRepository {
    constructor(
    ...
    async findById(id: string): Promise<KitShipmentStatusDocument | null> {
        return this.KitShipmentStatusModel.findOne({ _id: id, deleted_at: null }).exec()
    }
    ...
    async update(
        ...
        return this.KitShipmentStatusModel
            .findByIdAndUpdate(
                id,
                { ...updateKitShipmentStatusDto, updated_at: new Date(), updated_by: userId },
                { new: true },
            )
            .exec()
    }
    ...
    async delete(id: string, userId: string): Promise<boolean> {
        const result = await this.KitShipmentStatusModel
            .findByIdAndUpdate(
                id,
    ...
    async findByName(status: string): Promise<KitShipmentStatusDocument | null> {
        return this.KitShipmentStatusModel.findOne({ status, deleted_at: null }).exec()
    }
}
