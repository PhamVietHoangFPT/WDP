...
export class KitShipmentStatusService implements IKitShipmentStatusService {
  ...
  async findAll(): Promise<KitShipmentStatusResponseDto[]> {
    ...
    if (!types || types.length === 0) {
      throw new NotFoundException(
        'Không tìm thấy kiểu trạng thái vận chuyển nào.',
      )
    }
  ...
  async update(
    ...
    const existingName = await this.kitShipmentStatusRepository.findByName(
      data.status,
    )
    if (existingName) {
      throw new BadRequestException(
    ...
    const existingOrder = await this.kitShipmentStatusRepository.findByOrder(
      data.order,
    )
    if (existingOrder) {
      throw new BadRequestException(
    ...
    const updatedType = await this.kitShipmentStatusRepository.update(
      id,
      data,
      userId,
    )
