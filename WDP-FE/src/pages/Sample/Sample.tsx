...
const ManagerServiceCaseWithoutSampleCollector: React.FC = () => {
  ...
  const handleConfirmAssignment = async () => {
    ...
    try {
      ...
      message.success(
        `Đã gán nhân viên lấy mẫu ${selectedSampleCollector.name} cho dịch vụ thành công!`
      )
          ...
          items={[
            {
              key: 'no-collectors',
              label: (
                <span style={{ color: '#999' }}>Không có nhân viên nào</span>
              ),
              disabled: true,
            },
          ]}
  ...
  const columns: ColumnsType<ServiceCase> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Tag
          icon={isAtHome ? <HomeOutlined /> : <ShopOutlined />}
          color={isAtHome ? 'green' : 'blue'}
        >
        ...
        <div style={{ padding: '16px 0' }}>
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ marginLeft: '16px', marginTop: '8px' }}>
              ...
              <div>
                • Loại dịch vụ:{' '}
                {selectedServiceCase?.isAtHome ? 'Tại nhà' : 'Tại cơ sở'}
              </div>
              <div>
                • Ngày đặt:{' '}
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ marginLeft: '16px', marginTop: '8px' }}>
              ...
              {selectedSampleCollector?.addressInfo && (
                <div>• Địa chỉ: {selectedSampleCollector.addressInfo}</div>
              )}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ color: '#d46b08', marginTop: '4px' }}>
              Sau khi gán nhân viên lấy mẫu, dịch vụ sẽ được chuyển sang trạng
              thái "Đã có nhân viên phụ trách" và không thể hoàn tác.
            </div>
          </div>
        </div>
      </Modal>
