...
import { useGetKitShipmentForShipperQuery, useGetKitShipmentStatusQuery, useUpdateKitShipmentStatusMutation } from "../../features/kitShipment/kitShipmentAPI";
...
const DeliveryStaffKitShipment: React.FC = () => {
    const [selectedCurrentStatus, setSelectedCurrentStatus] = useState<string | undefined>(undefined);
    const { data, error, isLoading } = useGetKitShipmentForShipperQuery({ currentStatus: selectedCurrentStatus });
    ...
    const handleStatusUpdate = async (kitShipmentId: string, statusId: string, successMessage: string) => {
        try {
            await changeStatus({
    ...
    const handleConfirmDelivery = (kitShipmentId: string) =>
        handleStatusUpdate(kitShipmentId, STATUS_IDS.CONFIRMED, "Xác nhận giao hàng thành công!");

    const handleDeliverySuccess = (kitShipmentId: string) =>
        handleStatusUpdate(kitShipmentId, STATUS_IDS.DELIVERED, "Giao kit thành công!");
    ...
    const handleSampleSuccess = (kitShipmentId: string) =>
        handleStatusUpdate(kitShipmentId, STATUS_IDS.SAMPLE_SUCCESS, "Lấy mẫu thành công!");

    const handleSampleFailed = (kitShipmentId: string) =>
        handleStatusUpdate(kitShipmentId, STATUS_IDS.SAMPLE_FAILED, "Đã hủy do lấy mẫu thất bại!");

    const handleDeliveredToFacility = (kitShipmentId: string) =>
        handleStatusUpdate(kitShipmentId, STATUS_IDS.DELIVERED_TO_FACILITY, "Đã giao đến cơ sở thành công!");
    ...
    const renderActionButtons = (record: KitShipment) => {
        const status = kitShipmentStatus?.data?.find(
            (s: { _id: string; status: string; order: number }) => s._id === record.currentStatus
        );
    ...
    const columns: ColumnsType<KitShipment> = [
        {
            ...
            render: (_, record) => (
                <div>
                    <div style={{ fontWeight: "bold" }}>{record.caseMember.booking.account.name}</div>
        ...
        },
        {
            ...
            key: "bookingDate",
            render: (_, record) =>
                new Date(record.caseMember.booking.bookingDate).toLocaleDateString("vi-VN", {
                    day: "2-digit",
                    month: "2-digit",
                    year: "numeric",
                }),
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                return <Tag color="orange">{status?.status || "Chưa có nhân viên giao hàng"}</Tag>;
            },
        },
        {
            title: "Hành động",
            ...
            >
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <Select
                        ...
                        allowClear
                    >
                        {kitShipmentStatus?.data?.map((status: { _id: string; status: string; order: number }) => (
