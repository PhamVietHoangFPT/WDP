import { Card, Typography, Spin, Button, Image, Space, Row, Col, Descriptions, Tag, Table, Divider } from 'antd';
import { useParams } from 'react-router-dom';
import { useGetImageQuery, useGetServiceCaseByIdQuery } from '../../features/customer/paymentApi';
        ...
        <Col span={24}>
          <Card>
            ...
            <Descriptions bordered size="small" column={1}>
              <Descriptions.Item label="Mã hồ sơ">{serviceCase?._id || 'N/A'}</Descriptions.Item>
              <Descriptions.Item label="Ngày tạo">
                {serviceCase?.created_at ? new Date(serviceCase.created_at).toLocaleString('vi-VN') : 'N/A'}
              </Descriptions.Item>
              <Descriptions.Item label="Ngày đặt">
                {serviceCase?.caseMember?.booking?.bookingDate ? new Date(serviceCase.caseMember.booking.bookingDate).toLocaleDateString('vi-VN') : 'N/A'}
              </Descriptions.Item>
              <Descriptions.Item label="Trạng thái hiện tại">
                <Tag color="blue">{serviceCase?.currentStatus?.testRequestStatus || 'N/A'}</Tag>
              </Descriptions.Item>
            </Descriptions>
            ...
            <Descriptions bordered size="small" column={1}>
              <Descriptions.Item label="Tên tài khoản">{serviceCase?.account?.name || 'N/A'}</Descriptions.Item>
              <Descriptions.Item label="Số điện thoại">{serviceCase?.account?.phoneNumber || 'N/A'}</Descriptions.Item>
            </Descriptions>
              ...
              {serviceCase?.caseMember?.service?.length > 0 ? (
                serviceCase.caseMember.service.map((service: any) => (
                  <Descriptions.Item key={service._id} label="Dịch vụ">
                    {service.name} (Loại mẫu: {service.sample?.name || 'N/A'}) - Thời gian trả kết quả: {service.timeReturn?.timeReturn || 'N/A'} ngày
                  </Descriptions.Item>
                ))
              ) : (
            ...
            <Descriptions bordered size="small" column={1}>
              <Descriptions.Item label="Nhân viên lấy mẫu">
                {serviceCase?.sampleCollector ? (
                  <Space direction="vertical">
                    <Text strong>{serviceCase.sampleCollector.name || 'N/A'}</Text>
                    <Text type="secondary"><PhoneOutlined /> {serviceCase.sampleCollector.phoneNumber || 'N/A'}</Text>
                  </Space>
              ...
              <Descriptions.Item label="Bác sĩ phụ trách">
                {serviceCase?.doctor ? (
                  <Space direction="vertical">
                    ...
                    <Text type="secondary"><PhoneOutlined /> {serviceCase.doctor.phoneNumber || 'N/A'}</Text>
                  </Space>
                ) : <Tag>Chưa chỉ định</Tag>}
              </Descriptions.Item>
            </Descriptions>
            ...
            {serviceCase?.result ? (
              <>
                <Descriptions bordered size="small" column={1} style={{ marginBottom: 20 }}>
                  <Descriptions.Item label="Kết luận">{serviceCase.result.conclusion || 'N/A'}</Descriptions.Item>
                  <Descriptions.Item label="Phần trăm ADN phù hợp">{serviceCase.result.adnPercentage || 'N/A'}%</Descriptions.Item>
                  <Descriptions.Item label="Người xác nhận">{serviceCase.result.certifierId?.name || 'N/A'}</Descriptions.Item>
                </Descriptions>
                ...
                {serviceCase.adnDocumentation?.profiles?.length > 0 ? (
                  serviceCase.adnDocumentation.profiles.map((profile: any, index: number) => (
                    <div key={index} style={{ marginBottom: 20, border: '1px solid #f0f0f0', padding: 10 }}>
                      <Text strong>{profile.sampleIdentifyNumber || 'N/A'}</Text>
                      <Table
