...
const StaffGetServiceCaseByCustomer: React.FC = () => {
  ...
  const [updateServiceCaseStatus, { isLoading: isUpdatingStatus }] =
    useUpdateServiceCaseStatusForStaffMutation()
  ...
  const handleUpdateStatus = async (
    ...
    await handleUpdateServiceCaseStatus({
      id: serviceCaseId,
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        if (
          ...
          const checkinStatus = allStatuses?.find(
            (s) => s.testRequestStatus === 'Check-in'
          )
          if (checkinStatus) {
            availableStatuses.push(checkinStatus)
          }
        } else if (record.currentStatus === 'Check-in') {
          const choXuLyStatus = allStatuses?.find(
            (s) => s.testRequestStatus === 'Chờ xử lý'
          )
          if (choXuLyStatus) {
    ...
    <div style={{ padding: 24 }}>
      ...
      <Card style={{ marginBottom: 24 }}>
        <Form form={form} layout='vertical' onFinish={handleSearch}>
          ...
          <Form.Item name='currentStatus' label='Lọc theo Trạng Thái'>
            <Select
