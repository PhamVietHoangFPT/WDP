...
import { Typography, Spin, Alert, Space, Card, Button, Modal, Row, Col, notification } from 'antd'
import { EnvironmentOutlined, MailOutlined, PhoneOutlined, UserOutlined, InfoCircleOutlined, CompassOutlined, ReloadOutlined } from '@ant-design/icons' // Thêm ReloadOutlined
...
// Kiểu dữ liệu cho state của Modal Google Maps (vẫn giữ để code không lỗi)
...
export default function AdministrativeService() {
  ...
  // Danh sách cơ sở ban đầu từ API
  const facilities: FacilityDetail[] = facilitiesData?.data || []

  // State để lưu vị trí người dùng [latitude, longitude]
  const [userLocation, setUserLocation] = useState<[number, number] | null>(null)
  // State để lưu danh sách cơ sở đã được lọc/sắp xếp
  const [filteredFacilities, setFilteredFacilities] = useState<FacilityDetail[]>([])
  ...
  useEffect(() => {
    console.log("useEffect triggered: facilitiesData or userLocation changed");
    if (facilitiesData?.data) {
      if (userLocation) {
        console.log("User Location for filtering:", userLocation);
        const facilitiesWithDistance = facilitiesData.data.map((facility: FacilityDetail) => {
          if (facility.address?.location?.coordinates && facility.address.location.coordinates.length === 2) {
          ...
          console.warn(`Facility ${facility.facilityName} has invalid or missing coordinates. Setting distance to Infinity.`);
          return { ...facility, distance: Infinity }; // Nếu không có tọa độ hợp lệ, đặt khoảng cách vô cùng lớn
        });
        // Sắp xếp theo khoảng cách tăng dần
        const sortedFacilities = [...facilitiesWithDistance].sort((a, b) => (a.distance || Infinity) - (b.distance || Infinity));
        setFilteredFacilities(sortedFacilities);
        console.log("Filtered and Sorted Facilities:", sortedFacilities);
      } else {
        // Nếu không có vị trí người dùng, hiển thị danh sách gốc
        setFilteredFacilities(facilitiesData.data);
        console.log("Reset to Original Facilities:", facilitiesData.data);
      }
    } else {
      console.log("facilitiesData is not available yet.");
      setFilteredFacilities([]); // Đảm bảo rỗng nếu data chưa về
    }
  }, [facilitiesData, userLocation])

  // Modal state không còn được sử dụng cho Google Maps trong Card, nhưng vẫn giữ để code không lỗi
  ...
  const handleMapModalCancel = () => {
    setMapModal({ visible: false, facilityName: '', mapUrl: null })
  }
  const showMapModal = (facilityName: string, coordinates: number[]) => {
    const latitude = coordinates[1];
    const longitude = coordinates[0];
    // Sửa lại URL cho đúng định dạng Google Maps Embed API (thực ra không dùng modal này nữa)
      ...
      <div style={{ padding: 24, textAlign: 'center' }}>
        <Alert
          ...
          description={
            ...
            (facilitiesError.data ? JSON.stringify(facilitiesError.data) : "Đã có lỗi xảy ra khi tải danh sách cơ sở. Vui lòng thử lại.")
          }
          type="error"
          showIcon
          action={
  ...
  console.log("Rendering facilities:", facilitiesToDisplay); // Kiểm tra dữ liệu cuối cùng trước render


  return (
    <div style={{ padding: 24 }}>
      <Title level={2} style={{ color: '#0056b3', marginBottom: 20 }}>Dịch vụ Hành chính</Title>

      <Card style={{ marginBottom: 24, backgroundColor: '#e6f7ff', borderColor: '#91d5ff', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.09)' }}>
        <Paragraph style={{ fontSize: '16px', fontWeight: 'bold', color: '#1890ff', margin: 0 }}>
          <InfoCircleOutlined style={{ marginRight: 8, color: '#1890ff' }} />
          Các dịch vụ hành chính vui lòng liên hệ trực tiếp với quản lý của cơ sở tương ứng để được hỗ trợ.
        </Paragraph>
      </Card>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 30, marginBottom: 20 }}>
        <Title level={3} style={{ margin: 0, color: '#0056b3' }}>Thông tin liên hệ các Cơ sở</Title>
        <Space>
            {userLocation ? ( // Chỉ hiển thị nút reset khi đã có vị trí người dùng (đã lọc)
                <Button type="default" onClick={resetFilter} icon={<ReloadOutlined />}>
                    Hiển thị tất cả
                </Button>
            ) : null}
            <Button
      ...
      <Row gutter={[24, 24]} style={{ marginTop: 20 }}>
        {facilitiesToDisplay.length > 0 ? (
          facilitiesToDisplay.map((facility) => (
            <Col xs={24} sm={12} md={8} lg={6} key={facility._id}>
              <Card
                title={
                  <Space>
                    <Text strong ellipsis={{ tooltip: facility.facilityName }} style={{ fontSize: '1.1em', color: '#333' }}>
                ...
                bodyStyle={{ display: 'flex', flexDirection: 'column', flexGrow: 1, padding: '16px 24px' }}
                headStyle={{ backgroundColor: '#f0f2f5', borderBottom: '1px solid #e8e8e8', borderTopLeftRadius: 12, borderTopRightRadius: 12 }}
              >
                {/* Hiển thị khoảng cách nếu có userLocation */}
                {userLocation && facility.distance !== undefined && facility.distance !== Infinity ? (
                    <Paragraph style={{ marginBottom: 12, textAlign: 'center', fontSize: '0.9em', color: '#888' }}>
                        Cách bạn: <Text strong style={{ color: '#0056b3' }}>{facility.distance.toFixed(2)} km</Text>
                    </Paragraph>
                ) : null}

                {/* Phần bản đồ luôn ở trên cùng body của card */}
                {facility.address?.location?.coordinates && facility.address.location.coordinates.length === 2 ? (
                  <div style={{ marginBottom: 16, borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e8e8' }}>
                    <iframe
                  ...
                  <div style={{ marginBottom: 16, textAlign: 'center', padding: '20px 0', border: '1px dashed #d9d9d9', borderRadius: 8 }}>
                    <Text type="secondary" style={{ fontSize: '0.9em' }}>Không có vị trí bản đồ.</Text>
                  </div>
                )}
                ...
                <div style={{ minHeight: 70, marginBottom: 12, flexShrink: 0 }}>
                  <Paragraph style={{ marginBottom: 4 }}>
                    ...
                    <Text style={multiLineEllipsisStyle} title={facility.address?.fullAddress}>
                  ...
                  <Paragraph style={{ marginBottom: 0 }}>
                    ...
                    <Text style={singleLineEllipsisStyle} title={facility.phoneNumber}>
                ...
                <div style={{ flexGrow: 1, minHeight: 100, marginTop: 15, paddingLeft: 10, borderLeft: '3px solid #f0f2f5' }}>
                  <Text strong style={{ display: 'block', marginBottom: 8, color: '#333' }}><UserOutlined style={{ marginRight: 8, color: '#eb2f96' }} /> Nhân viên Quản lý:</Text>
                  {facility.account ? (
                    <Space direction="vertical" size={4} style={{ width: '100%', paddingLeft: 12 }}>
                      {facility.account.name && (
                        <Paragraph style={{ marginBottom: 2 }}>
                          <Text strong style={singleLineEllipsisStyle} title={facility.account.name}>
                            {facility.account.name}
                          </Text>
                        </Paragraph>
                      )}
                      <Paragraph style={{ marginBottom: 2 }}>
                        ...
                        <Text type="secondary" style={singleLineEllipsisStyle} title={facility.account.email}>
                          {facility.account.email}
                        </Text>
                      </Paragraph>
                      <Paragraph style={{ marginBottom: 0 }}>
                        <PhoneOutlined style={{ marginRight: 8, color: '#faad14' }} />
                        <Text type="secondary" style={singleLineEllipsisStyle} title={facility.account.phoneNumber}>
                ...
                <div style={{ borderTop: '1px solid #f0f0f0', paddingTop: 15, textAlign: 'center' }}>
                    <Text type="secondary" style={{ fontSize: '0.85em' }}>Thông tin được cập nhật thường xuyên.</Text>
                </div>
              </Card>
            </Col>
          ))
          ...
          <Col span={24} style={{ textAlign: 'center', padding: '50px 0' }}>
            <Text type="secondary">Không có dữ liệu cơ sở nào để hiển thị.</Text>
          </Col>
        )}
      </Row>

      {/* Modal hiển thị Google Maps (giữ lại định nghĩa nhưng không dùng) */}
      <Modal
        title={`Vị trí: ${mapModal.facilityName}`}
      ...
      >
        {mapModal.mapUrl ? (
          <iframe
            ...
            loading="lazy"
            referrerPolicy="no-referrer-when-downgrade"
            title={`Google Map for ${mapModal.facilityName}`}
          ></iframe>
        ) : (
